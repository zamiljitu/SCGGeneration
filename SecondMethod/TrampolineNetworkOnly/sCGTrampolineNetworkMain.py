...
class gui_tk(Tkinter.Tk):
    ...
    def initialize(self):
        ...
        #self.lblNumEdgesText.set(u'No. of Additional Edges between Trampolines')
        ...
        buttonCreateCG = Tkinter.Button(self,text=u'Generate Trampoline', 
                                              command=self.onCreateTrampolinesClick)
    ...
    def onViewCGClick(self):
        ...
        if self.CG:
            self.tree.plotGraph(self.tree.trampolinesConnected, 1)
        else:
    ...
    def onViewSCGClick(self):
        ...
        if self.SCG:
            self.tree.plotGraph(self.tree.stronglyChordalGraphDict, 2)
        else:
...
if __name__ == "__main__":
    ...
    app.geometry('350x75')#window size
    center(app)
    app.mainloop()
    app.quit()